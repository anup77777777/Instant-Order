...
#Owner name
ownername=CTkLabel(win,
                text="Owner's Name:",
                bg_color='lightgray',
                font=("Arial", 30, 'bold'),
                text_color= 'black')
ownername.place(x=940, y=175)
ownername_entry = CTkEntry(win,
                           font=("Arial", 25,),
                           bg_color='lightgray',
                           height=40,
                           width=250,
                           ...
                           border_width=3,
                           border_color='#d40707')
ownername_entry.place(x=1200, y=175)

#Owner Number
ownernumber=CTkLabel(win,
                text="Owner's Number:",
                bg_color='lightgray',
                font=("Arial", 30,'bold'),
                text_color= 'black')
ownernumber.place(x=940, y=275)

def number():
    input= ownernumber_entry.get()
    if len(input)==10 and input.isdigit():
        return True
    else:
        messagebox.showerror("Error","Phone number should be 10 numbers only")
        return False
ownernumber_entry = Entry(win,
                           font=('Arial', 25),
                                 width=17,
                                 ...
                                 validate='focusout')
ownernumber_entry.place(x=1500, y=345)
...
def register_click():
    ...
    cursor.execute("INSERT INTO owners_information VALUES (:ownername, :ownernumber, :username, :password)",
                   {
                       'ownername': ownername_entry.get(),
                       'ownernumber': ownernumber_entry.get(),
    ...
    if ownername_entry.get()=='' or ownernumber_entry.get()=='' or username_entry.get()=='' or password_entry.get()=='':
